...
def main_fn(input_sdf, output_sdf, init_sdf):
    ...
    if input_sdf_path.suffix == ".pkl":
        ...
        raise ValueError(
            f"Unsupported input file format: {input_sdf_path.suffix}. Only .sdf and .pkl files are supported."
        )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        try:
            for task in tqdm(enumerate(suppl)):
                optimized_mol, total_energy_gain, total_rmsd = process_molecule(
                    task, temp_dir
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_sdf", type=str, required=True, help="Path to input .sdf file"
    )
    parser.add_argument(
        "--output_sdf", type=str, required=True, help="Path to output optimized .sdf"
    )
    parser.add_argument(
        "--init_sdf",
        type=str,
        required=True,
        help="Path to output initial structures .sdf",
    )
