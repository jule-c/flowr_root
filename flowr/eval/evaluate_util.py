...
def gather_predictions_pilot(args, multiple_files=False):
    """
    Reads a 'predictions.pt' file and returns three lists:
    gen_ligs, ref_ligs_with_hs, ref_pdbs_with_hs
    """
    if multiple_files:
        predictions = glob(os.path.join(args.save_dir, "predictions_*.pt"))
        if not args.valid_unique:
            predictions = [pred for pred in predictions if "valid_unique" not in pred]
        else:
            predictions = [pred for pred in predictions if "valid_unique" in pred]
        if not predictions:
        ...
        for pred in predictions:
            pred = torch.load(pred, map_location="cpu", weights_only=False)
            gen_ligs_, ref_ligs_, ref_ligs_with_hs_, ref_pdbs_, ref_pdbs_with_hs_ = (
                extract_batch_multi(args, pred, pilot=True)
            )
            gen_ligs.extend(gen_ligs_)
            ref_ligs.extend(ref_ligs_)
            ref_ligs_with_hs.extend(ref_ligs_with_hs_)
            ref_pdbs.extend(ref_pdbs_)
            ref_pdbs_with_hs.extend(ref_pdbs_with_hs_)
            time_per_pocket.extend(pred["time_per_pocket"])
        print(
            f"Average sampling time per pocket: {np.mean(time_per_pocket)} +- {np.std(time_per_pocket)}"
        )
    else:
        gen_sdf_files = glob(os.path.join(args.save_dir, "sampled", "*.sdf"))
        ref_sdf_files = []
        ref_sdf_files_with_hs = []
        ref_pdbs = []
        ref_pdbs_with_hs = []
        for sdf_file in gen_sdf_files:
            pdb_name = (Path(sdf_file).name).replace("_gen.sdf", ".pdb")
            sdf_name = (Path(sdf_file).name).replace("_gen.sdf", ".sdf")
            ref_sdf_files.append(
                os.path.join(args.data_path, "pilot_test_sdfs", sdf_name)
            )
            ref_sdf_files_with_hs.append(
                os.path.join(args.data_path, "pilot_test_sdfs_with_hs", sdf_name)
            )
            ref_pdbs.append(os.path.join(args.data_path, "pilot_test_pdbs", pdb_name))
            ref_pdbs_with_hs.append(
                os.path.join(args.data_path, "pilot_test_pdbs_with_hs", pdb_name)
            )

        gen_ligs = [
            [lig for lig in Chem.SDMolSupplier(sdf_file, removeHs=False)]
            for sdf_file in gen_sdf_files
        ]
        ref_ligs = [Chem.SDMolSupplier(sdf_file)[0] for sdf_file in ref_sdf_files]
        ref_ligs_with_hs = [
            Chem.SDMolSupplier(sdf_file, removeHs=False)[0]
            for sdf_file in ref_sdf_files_with_hs
        ]

    assert (
        len(gen_ligs)
        == len(ref_ligs)
        == len(ref_ligs_with_hs)
        == len(ref_pdbs)
        == len(ref_pdbs_with_hs)
    ), "All lists must have the same length."

    return gen_ligs, ref_ligs, ref_ligs_with_hs, ref_pdbs, ref_pdbs_with_hs


def gather_predictions(args, multiple_files=False):
    """
    Reads a 'predictions.pt' file and returns three lists:
    gen_ligs, ref_ligs_with_hs, ref_pdbs_with_hs
    """
    if multiple_files:
        predictions = glob(os.path.join(args.save_dir, "predictions_multi_*.pt"))
        if not args.valid_unique:
            predictions = [pred for pred in predictions if "valid_unique" not in pred]
        else:
            predictions = [pred for pred in predictions if "valid_unique" in pred]
        ...
        predictions = torch.load(
            Path(args.save_dir) / "predictions.pt", map_location="cpu"
        )
    ...
    if multiple_files:
        ...
        for pred in predictions:
            ...
            ref_pdbs_with_hs.extend(ref_pdbs_with_hs_)
            time_per_pocket.extend(pred["time_per_pocket"])
            time_per_complex.extend(pred["time_per_complex"])
        print(
            f"Average sampling time per pocket: {np.mean(time_per_pocket)} +- {np.std(time_per_pocket)}"
        )
        print(
            f"Average sampling time per complex: {np.mean(time_per_complex)} +- {np.std(time_per_complex)}"
        )
    else:
        gen_ligs, ref_ligs, ref_ligs_with_hs, ref_pdbs, ref_pdbs_with_hs = (
            extract_batch(args, predictions)
        )

    return gen_ligs, ref_ligs, ref_ligs_with_hs, ref_pdbs, ref_pdbs_with_hs


def extract_batch_multi(args, predictions, pilot=False):
    ligs_by_pdb = defaultdict(create_list_defaultdict)
    ligs_by_pdb_with_hs = defaultdict(create_list_defaultdict)
    for gen_ligs, ref_lig, ref_lig_with_hs, pdb, pdb_with_hs in zip(
        predictions["gen_ligs"],
        predictions["ref_ligs"],
        predictions["ref_ligs_with_hs"],
        predictions["ref_pdbs"],
        predictions["ref_pdbs_with_hs"],
    ):
        assert Path(pdb).exists(), f"File {pdb} does not exist."
        assert Path(pdb_with_hs).exists(), f"File {pdb_with_hs} does not exist."
        ligs_by_pdb[pdb]["gen"].extend(gen_ligs)
        ligs_by_pdb[pdb]["ref"] = ref_lig
        ligs_by_pdb_with_hs[pdb_with_hs]["ref"] = ref_lig_with_hs
    gen_ligs = [v["gen"] for v in ligs_by_pdb.values()]
...
def extract_batch(args, predictions):
    ligs_by_pdb = defaultdict(create_list_defaultdict)
    ligs_by_pdb_with_hs = defaultdict(create_list_defaultdict)
    for batch in predictions:
        for gen_ligs, ref_lig, ref_lig_with_hs, pdb, pdb_with_hs in zip(
            batch["gen_ligs"],
            batch["native_ligs"],
            batch["native_ligs_with_hs"],
            batch["ref_pdbs"],
            batch["ref_pdbs_with_hs"],
        ):
            if Path(pdb).parent != Path(args.data_path) / "ref_test_pdbs":
                pdb = os.path.join(
                    Path(args.data_path) / "ref_test_pdbs", Path(pdb).name
                )
                pdb_with_hs = os.path.join(
                    args.data_path, "ref_test_pdbs", Path(pdb_with_hs).name
                )
                assert Path(pdb).exists(), f"File {pdb} does not exist."
                assert Path(pdb_with_hs).exists(), f"File {pdb_with_hs} does not exist."
            ligs_by_pdb[pdb]["gen"].extend(gen_ligs)
...
def gather_predictions_mol(args, multiple_files=False):
    """
    Reads a 'predictions.pt' file and returns a list of all generated molecules:
    """
    if multiple_files:
        predictions = glob(os.path.join(args.save_dir, "predictions_multi_*.pt"))
        if not args.valid_unique:
...
def chunkify(lst, num_chunks):
    """
    Splits list `lst` into `num_chunks` (almost) equal pieces.
    """
...
class MolPropertyEvaluator(AbstractEvaluator):
    ...
    def evaluate(self, molecule, protein=None):
        molecule = self.load_molecule(molecule)
        return {
            k: v for k, v in molecule.GetPropsAsDict().items() if isinstance(v, float)
        }
...
class PoseBustersEvaluator(AbstractEvaluator):
    ...
    def evaluate(self, molecule, protein=None):
        result = safe_run(
            self.posebusters.bust, timeout=60, mol_pred=molecule, mol_cond=protein
        )
        if result is None:
            return dict()
        ...
        result["all"] = all(
            [bool(value) if not is_nan(value) else False for value in result.values()]
        )
        return result
...
class GeometryEvaluator(AbstractEvaluator):
    ...
    def get_angle_values(mol, triplets):
        ...
        angles = np.arccos(
            (vji * vjk).sum(axis=1)
            / (np.linalg.norm(vji, axis=1) * np.linalg.norm(vjk, axis=1))
        )
...
class MedChemEvaluator(AbstractEvaluator):
    ...
    def evaluate(self, molecule, protein=None):
        ...
        n_rotatable_bonds = (
            None if not valid else self.calculate_rotatable_bonds(molecule)
        )
    ...
    def is_connected(self, rdmol):
        ...
        try:
            ...
            return (
                largest_frag.GetNumAtoms() / rdmol.GetNumAtoms()
...
class REOSEvaluator(AbstractEvaluator):
    ...
    def evaluate(self, molecule, protein=None):
        ...
        results["all"] = all(
            [bool(value) if not is_nan(value) else False for value in results.values()]
        )
        return results
...
class FCDEvaluator(AbstractCollectionEvaluator):
    ...
    def evaluate(self, smiles: Collection[str]):
        if len(smiles) > len(self.reference_smiles):
            print(
                "Number of reference molecules should be greater than number of input molecules"
            )
            return {"fcd": None}

        np.random.seed(42)
        reference_smiles = np.random.choice(
            self.reference_smiles, len(smiles), replace=False
        ).tolist()
...
class RingDistributionEvaluator(AbstractCollectionEvaluator):
    ...
    def compute_ring_dict(self, molecules):
        ...
        for mol in tqdm(molecules, desc="Computing ring systems"):
            ...
            try:
                ring_system_list = self.ring_system_finder.find_ring_systems(
                    mol, as_mols=True
                )
            except ValueError:
                print(
                    f"WARNING[{type(self).__name__}]: error while computing ring systems; skipping molecule."
                )
                continue
    ...
    def precision(self, query_ring_dict):
        ...
        return (
            len(intersection) / len(query_ring_systems)
    ...
    def recall(self, query_ring_dict):
        ...
        intersection = ref_ring_systems & query_ring_systems
        return (
            len(intersection) / len(ref_ring_systems)
            if len(ref_ring_systems) > 0
            else 0
        )
    ...
    def jsd(self, query_ring_dict, k_most_freq=None):
        ...
        if k_most_freq is None:
            ...
            sorted_rings = [
                k
                for k, v in sorted(
                    self.ref_ring_dict.items(), key=lambda item: item[1], reverse=True
                )
            ]
    ...
    def evaluate(self, smiles: Collection[str]):
        ...
        out.update(
            {
                f"jsd_{k}_most_freq": self.jsd(query_ring_dict, k_most_freq=k)
                for k in self.jsd_on_k_most_freq
            }
        )
    ...
    def __init__(
        ...
        self.evaluators = [
            ...
            RingDistributionEvaluator(
                reference_smiles, jsd_on_k_most_freq=[10, 100, 1000, 10000]
            ),
        ]
        for e in self.evaluators:
            if e.ID in exclude_evaluators:
...
def combine_drugflow_samples():
    # Input directory with drugflow samples.
    input_dir = Path("/hpfs/userws/cremej01/projects/drugflow_samples")
    # Output directory where results will be stored.
    output_dir = Path("/hpfs/userws/cremej01/projects/drugflow_eval")
    output_dir.mkdir(exist_ok=True)

    # Iterate over every subdirectory in the input folder.
    for subfolder in input_dir.iterdir():
        if subfolder.is_dir():
            subfolder_name = subfolder.name
            print(f"Processing folder: {subfolder_name}")

            # Create output filenames based on the subfolder name.
            # The ligand SDF file: keep the entire subfolder name and add .sdf.
            ligand_sdf_out = output_dir / f"{subfolder_name}.sdf"
            # The pocket PDB file: use the part of the folder name before the first underscore.
            new_pdb_name = subfolder_name.split("_")[0] + ".pdb"
            pocket_pdb_out = output_dir / new_pdb_name

            # Process ligand SDF files: these are named like i_ligand.sdf (i in [0, 100])
            ligand_files = sorted(list(subfolder.glob("*_ligand.sdf")))
            if ligand_files:
                writer = Chem.SDWriter(str(ligand_sdf_out))
                for sdf_file in ligand_files:
                    ...
                    for mol in suppl:
                        if mol is not None:
                            writer.write(mol)
                writer.close()
                print(f"Wrote combined ligands: {ligand_sdf_out}")
            else:
                print(f"No ligand SDF files found in {subfolder}")

            # Process pocket PDB files: these are named like i_pocket.pdb.
            pocket_files = sorted(list(subfolder.glob("*_pocket.pdb")))
            if pocket_files:
                # Take the first pocket pdb file.
                first_pocket = pocket_files[0]
                # Copy it to the output directory with new name.
                with open(first_pocket, "r") as infile, open(
                    pocket_pdb_out, "w"
                ) as outfile:
                    outfile.write(infile.read())
                print(f"Copied pocket PDB: {pocket_pdb_out}")
            else:
                print(f"No pocket PDB files found in {subfolder}")
