...
def get_all_protein_atoms(input_pdb_filepath: str):
...
def save_biopython_structure_to_pdb(
    structure, output_pdb_filepath: str, ca_only: bool = False
):
...
def compute_protein_local_geometry_violations(
    ...
    for bond_index in bond_index_table:
        ...
        if violation:
            violation_residue_idx.extend(
                [atom1.get_full_id()[3][1], atom2.get_full_id()[3][1]]
            )
...
def compute_ligand_local_geometry_violations(
    ...
    sdf_filepath = (
        input_sdf_filepath if input_sdf_filepath is not None else ref_input_sdf_filepath
    )
    ...
    for ligand_frag in ligand_frags:
        ...
        deviation_greater_than_cutoff = (
            abs(
                ref_pair_distances[local_geometry_mask]
                - pair_distances[local_geometry_mask]
            )
            > 0.3
        ).sum()
    ...
    try:
        if not temp_file.exists():
            ...
            with open(temp_file, "w", encoding="utf-8") as f:
                PDBFile.writeFile(
                    fixer.topology, fixer.positions, f, keepIds=relax_protein
                )
    ...
    if cache_dir is not None:
        ...
        if system_path.exists():
            ...
            if system.getNumParticles() == num_particles_total:
                print(f"Loaded system from cache: {system_path}")
                return system

    print(f"Generating system for {name}")
...
def optimize_ligand_in_pocket(
    ...
    for ligand_index, ligand in enumerate(ligand_frags, start=1):
        ...
        if relax_protein or assign_partial_charges_manually:
            try:
                ...
                logger.warning(
                    f"Unable to assign partial charges to {ligand_file} due to: {e}"
                )
    ...
    num_particles_ligand = sum(
        [len(ligand.conformers[0].magnitude) for ligand in ligands]
    )
    ...
    if platform.getName() == "CUDA":
        ...
        print(f"Using platform: {platform.getName()}:{cuda_device_index}")
    else:
    ...
    if relax_protein:
        ...
        force = CustomExternalForce(
            "0.5 * protein_k * ((x-x0)^2 + (y-y0)^2 + (z-z0)^2)"
        )
        ...
        for i, atom in enumerate(reference_modeller.topology.atoms()):
            if (
                atom.residue.index < num_residues
                and protein_gap_mask[atom.residue.index] == "1"
            ):
                continue
        ...
        if assign_each_ligand_unique_force:
            ...
            ligand_force = CustomExternalForce(
                "0.5 * ligand_k * ((x-x0)^2 + (y-y0)^2 + (z-z0)^2)"
            )
    ...
    if relax_protein:
        ...
        results_dict["pos_init"] = state_before.getPositions(
            asNumpy=True
        ...
        results_dict["pos_final"] = state_after.getPositions(
            asNumpy=True
        ).value_in_unit(length)
        ...
        if protein_file.suffix == ".pdb":
            ...
            remove_hydrogens_from_pdb(
                str(protein_output_file), str(protein_output_file)
            )
            # record complex PDB file
            if complex_output_file is not None:
                with open(complex_output_file, "w") as f:
                    PDBFile.writeFile(
                        modeller.topology, results_dict["pos"], f, keepIds=True
                    )
        ...
        if output_file is not None:
            ...
            for ligand_index in range(len(ligands)):
                ...
                new_molecule.add_conformer(
                    openff_Quantity(
                        results_dict["pos_final"][pos_start_index:pos_end_index],
                        units="angstrom",
                    )
                )
        ...
        if output_file is not None:
            save_with_rdkit(
                ligand, output_file, conformer_index=1, name=name, mol=ligand
            )
...
def minimize_energy(
    ...
    # Name of protein as PDB ID
    name = Path(pdb_file).stem

    # Create a temporary directory with tempfile
    temp_directory = Path(tempfile.mkdtemp())

    # Create protein output file in output directory
    protein_output_file = Path(output_dir) / f"{name}_protein_{idx}.pdb"
    # Create complex output file in output directory
    complex_output_file = Path(output_dir) / f"{name}_complex_{idx}.pdb"

    protein_file_path = Path(pdb_file)
    ligand_file_path = Path(ligand_file)
    output_file_path = Path(output_dir) / f"{name}_min_ligand_{idx}.sdf"
    protein_output_file_path = (
        Path(protein_output_file) if protein_output_file else None
    )
    complex_output_file_path = (
        Path(complex_output_file) if complex_output_file else None
    )
    ...
    if relax_protein:
        ...
        temp_protein_file_path = Path(
            str(protein_file_path).replace(".pdb", "_temp.pdb")
        )
        ...
        print("Performing iteration 0 of complex relaxation")
    ...
    try:
        ...
        results_dict = optimize_ligand_in_pocket(
            ...
            relax_protein=relax_protein,
            remove_initial_protein_hydrogens=remove_initial_protein_hydrogens,
            assign_each_ligand_unique_force=assign_each_ligand_unique_force,
            report_initial_energy_only=report_initial_energy_only,
            model_ions=model_ions,
            cache_files=cache_files,
            assign_partial_charges_manually=assign_partial_charges_manually,
            tolerance=2.39 if relax_protein else 0.01,
        )
    except Exception as e:
        print(
        ...
        print(f"Finalizing complex relaxation with relax_protein={relax_protein}")
        ...
        if relax_protein:
            ...
            temp_name = temp_protein_file_path.stem if name is None else name
            try:
    ...
    if not relax_protein:
        if report_initial_energy_only:
            ...
            print(f"{ligand_file_path}, " + f"E_start: {energy_before:.2f} kJ/mol, ")
        else:
            energy_before = results_dict["e_init"].value_in_unit(kilojoule / mole)
        ...
        try:
            ...
            while results_dict["e_final"] > 0 and num_attempts < max_num_attempts:
                print(f"Performing iteration {num_attempts + 1} of complex relaxation")
                results_dict = optimize_ligand_in_pocket(
                    protein_file=temp_protein_file_path,
                    ligand_file=temp_ligand_file_path,
                    output_file=output_file_path,
            ...
            protein_score, violation_residue_idx = (
                compute_protein_local_geometry_violations(str(temp_protein_file_path))
            )
            num_attempts = 0
            while (
                ...
                save_biopython_structure_to_pdb(
            ...
            if (
                protein_score > 0
                or ligand_score > 0
                or results_dict["e_final"] > max_final_e_value
            ):
                print(
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Minimize the energy of a ligand in a protein pocket using OpenMM."
    )
    parser.add_argument("ligand_file", type=str, help="Path to the ligand file.")
    parser.add_argument("protein_file", type=str, help="Path to the protein file.")
    parser.add_argument("output_dir", type=str, help="Path to the output directory.")
    parser.add_argument(
        "--platform",
        type=str,
        default="fastest",
        help="The name of the OpenMM platform to use.",
    )
    parser.add_argument(
        "--relax_protein",
        action="store_true",
        help="Whether or not to relax the protein.",
    )
    parser.add_argument(
        "--remove_initial_protein_hydrogens",
        action="store_true",
        help="Whether or not to remove the initial protein hydrogens.",
    )
    parser.add_argument(
        "--assign_each_ligand_unique_force",
        action="store_true",
        help="Whether to assign each ligand a unique force constant.",
    )
    parser.add_argument(
        "--report_initial_energy_only",
        action="store_true",
        help="Whether or not to report the initial energy only.",
    )
    parser.add_argument(
        "--model_ions", action="store_true", help="Whether or not to model ions."
    )
    parser.add_argument(
        "--cache_files",
        action="store_true",
        help="Whether or not to cache the prepared files.",
    )
    parser.add_argument(
        "--assign_partial_charges_manually",
        action="store_true",
        help="Whether or not to assign partial charges manually.",
    )
    parser.add_argument(
        "--max_num_attempts",
        type=int,
        default=5,
        help="The maximum number of attempts to relax the protein.",
    )
    parser.add_argument(
        "--max_final_e_value",
        type=int,
        default=1000.0,
        help="The maximum final energy value.",
    )
    parser.add_argument(
        "--log_level", type=str, default="INFO", help="The logging level."
    )
    parser.add_argument(
        "--cuda_device_index",
        type=int,
    ...
    parser.add_argument(
        "--add_solvent",
        action="store_true",
        help="Whether or not to add solvent to the protein.",
    )
    ...
    minimize_energy(
        ...
        pdb_file=args.protein_file,
        output_dir=args.output_dir,
        platform=args.platform,
        relax_protein=args.relax_protein,
        remove_initial_protein_hydrogens=args.remove_initial_protein_hydrogens,
        assign_each_ligand_unique_force=args.assign_each_ligand_unique_force,
        report_initial_energy_only=args.report_initial_energy_only,
        model_ions=args.model_ions,
        cache_files=args.cache_files,
        assign_partial_charges_manually=args.assign_partial_charges_manually,
        max_num_attempts=args.max_num_attempts,
        max_final_e_value=args.max_final_e_value,
