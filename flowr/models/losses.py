...
class LossComputer:
    ...
    def __init__(
        ...
        self.mask_fixed_discrete_loss = mask_fixed_discrete_loss
        self.t_loss_weights = (
            TimeLossWeighting(
                max_value=100,
                min_value=0.05,
    ...
    def compute_losses(
        ...
        # Build discrete loss mask: exclude fixed atoms from discrete losses
        discrete_mask = mask
        if self.mask_fixed_discrete_loss and inpaint_mask is not None:
            discrete_mask = mask & (~inpaint_mask.bool())
            # If no variable atoms remain, use full mask to avoid division by zero
            if not discrete_mask.any():
                discrete_mask = mask
        ...
        # Other losses (use discrete_mask to exclude fixed atoms from discrete losses)
        type_loss = self.compute_type_loss(
            data, predicted, discrete_mask, t_loss_weights_disc
        )
        charge_loss = self.compute_charge_loss(
            data, predicted, discrete_mask, t_loss_weights_disc
        )
        bond_loss = (
            self.compute_bond_loss(data, predicted, discrete_mask, t_loss_weights_disc)
            if "bonds" in predicted
            else None
        )
        hybridization_loss = (
            self.compute_hybridization_loss(
                data, predicted, discrete_mask, t_loss_weights_disc
            )
