...
class Integrator:
    ...
    def mean_flow_step(
        ...
        """Perform a MeanFlow integration step.

        For MeanFlow, the model predicts data (x_1) with a u-head.
        We derive velocity and step forward.

        Args:
            curr: Current state dict
            predicted: Predicted data dict (softmax probabilities for discrete)
            prior: Prior noise dict
            times: [lig_times_cont, lig_times_disc]
        ...
        # Get per-sample time (handle both [B] and [B, N, 1] formats)
        if lig_times_cont.dim() == 1:
            t = lig_times_cont.view(B, 1, 1)
        elif lig_times_cont.dim() == 3:
            t = lig_times_cont[:, 0:1, :]  # [B, 1, 1]
        else:
            t = lig_times_cont.unsqueeze(-1).unsqueeze(-1)

        denom = (1 - t).clamp(min=1e-5)

        # Coordinate velocity and step
        velocity = (predicted["coords"] - curr["coords"]) / denom
        coords = curr["coords"] + step_size * velocity
        coords = coords * mask.unsqueeze(-1)

        # Discrete features: velocity-based stepping with softmax projection
        denom_disc = denom
        atomics_vel = (predicted["atomics"] - curr["atomics"]) / denom_disc
        atomics = curr["atomics"] + step_size * atomics_vel
        atomics = torch.nn.functional.softmax(atomics, dim=-1)
        atomics = atomics * mask.unsqueeze(-1)

        charges_vel = (predicted["charges"] - curr["charges"]) / denom_disc
        charges = curr["charges"] + step_size * charges_vel
        charges = torch.nn.functional.softmax(charges, dim=-1)
        charges = charges * mask.unsqueeze(-1)
        ...
        if "bonds" in predicted:
            denom_bond = (1 - t.unsqueeze(-1)).clamp(min=1e-5)
            bonds_vel = (predicted["bonds"] - curr["bonds"]) / denom_bond
            bonds = curr["bonds"] + step_size * bonds_vel
            bonds = torch.nn.functional.softmax(bonds, dim=-1)
            adj = smolF.adj_from_node_mask(mask)
            bonds = bonds * adj.unsqueeze(-1)
            bonds = 0.5 * (bonds + bonds.transpose(1, 2))
            updated["bonds"] = bonds.float()

        if "hybridization" in predicted:
            hyb_vel = (predicted["hybridization"] - curr["hybridization"]) / denom_disc
            hyb = curr["hybridization"] + step_size * hyb_vel
            updated["hybridization"] = torch.nn.functional.softmax(
                hyb, dim=-1
            ) * mask.unsqueeze(-1)

        if "fragment_mask" in curr:
            updated["fragment_mask"] = curr["fragment_mask"]
