...
class _InvariantEmbedding(torch.nn.Module):
    def __init__(
        ...
        if sc_charges and n_charge_sc_types > 0:
            atom_in_feats += n_charge_sc_types
    ...
    def forward(
        ...
        if self.sc_charges and cond_charges is not None:
            invs = torch.cat((invs, cond_charges), dim=-1)
...
class RadialBasisEmbedding(nn.Module):
    ...
    def forward(
        ...
        # NOTE: We use a manual implementation instead of torch.cdist because
        # torch.cdist does not support forward-mode AD (torch.func.jvp), which
        # is needed by MeanFlow training.
        # Manual: ||x_i - x_j||^2 = ||x_i||^2 + ||x_j||^2 - 2 * x_i · x_j
        sq_norms = (coords * coords).sum(dim=-1)  # [B, N]
        dot_products = torch.bmm(coords, coords.transpose(1, 2))  # [B, N, N]
        sq_dists = sq_norms.unsqueeze(2) + sq_norms.unsqueeze(1) - 2 * dot_products
        distances = torch.sqrt(sq_dists.clamp(min=1e-12))  # [B, N, N]
