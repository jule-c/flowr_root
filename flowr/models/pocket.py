...
class LigandDecoder(torch.nn.Module):
    ...
    def __init__(
        ...
        # Mean flow delta_t
        use_mean_flow_delta_t: bool = False,
        # Numerical stability
        eps: float = 1e-6,
    ):
        super().__init__()
        ...
        self.sc_distance_edges = sc_distance_edges
        self.eps = eps
        ...
        self.use_mean_flow_delta_t = use_mean_flow_delta_t
        ...
        # Add is_fixed embeddings for per-atom fixed/variable conditioning
        if self.use_is_fixed_embed:
            self.is_fixed_node_embed = torch.nn.Embedding(2, d_inv)
            self.is_fixed_position_embed = torch.nn.Embedding(2, d_inv)
        ...
        self.inv_emb = _InvariantEmbedding(
            ...
            n_charge_sc_types=n_charge_types if sc_charges else 0,
        )
        ...
        # *** Distance edge self-conditioning *** #
        if self.sc_distance_edges:
            from flowr.models.self_conditioning import DistanceEdgeSelfCond

            # d_edge must match rbf_embeds dim (original emb_size, before doubling)
            _sc_edge_dim = emb_size // 2 if self.use_rbf else emb_size
            self.dist_edge_sc = DistanceEdgeSelfCond(
                num_rbf=16, cutoff=5.0, d_edge=_sc_edge_dim
            )
        ...
        # *** Delta t embedding (for mean flow) *** #
        if self.use_mean_flow_delta_t:
            if use_fourier_time_embed:
                self.delta_t_fourier = TimeFourierEncoding(
                    posenc_dim=d_equi, max_len=200, random_permute=False
                )
                self.delta_t_emb = MLP(d_equi, d_equi)
            else:
                self.delta_t_emb = MLP(1, d_equi)
        else:
    ...
    def hparams(self):
        return {
            ...
            "use_mean_flow_delta_t": self.use_mean_flow_delta_t,
            "sc_charges": self.sc_charges,
            "sc_distance_edges": self.sc_distance_edges,
        }
    ...
    def forward(
        ...
        Args:
            ...
            mean_flow_delta_t (torch.Tensor): Delta t for mean flow, shape [B] or [B, N, 1]
        ...
        # Integrate distance edge self-conditioning
        if self.sc_distance_edges and cond_dist_edges is not None:
            if rbf_embeds is not None:
                rbf_embeds = rbf_embeds + cond_dist_edges
            else:
                rbf_embeds = cond_dist_edges
        ...
        # Embed delta_t for mean flow
        # Always compute MLP(delta_t) when enabled, defaulting to delta_t=0
        # This ensures consistency: MLP(0) during FM mode, not zero tensor
        delta_t_emb = torch.zeros_like(times_cont)
        if self.use_mean_flow_delta_t:
            # mean_flow_delta_t: [B] or [B, N, 1], default to zeros
            if mean_flow_delta_t is None:
                mean_flow_delta_t = torch.zeros(
                    coords.size(0), device=coords.device, dtype=coords.dtype
                )
            if mean_flow_delta_t.dim() == 1:
                _delta_t = mean_flow_delta_t.view(-1, 1, 1).expand(
                    -1, coords.size(1), -1
                )
            else:
                _delta_t = mean_flow_delta_t
            if self.use_fourier_time_embed:
                delta_t_emb = self.delta_t_fourier(_delta_t)
                delta_t_emb = self.delta_t_emb(delta_t_emb)
            else:
                delta_t_emb = self.delta_t_emb(_delta_t)

        equis = (
            self.coord_emb(_coords)
            + times_cont.unsqueeze(2).expand(-1, -1, 3, -1)
            + delta_t_emb.unsqueeze(2).expand(-1, -1, 3, -1)
        ...
        # Add is_fixed embeddings for per-atom conditioning
        if self.use_is_fixed_embed:
            B, N = coords.shape[:2]
            if is_fixed_node is None:
                is_fixed_node = torch.zeros(
                    B, N, dtype=torch.long, device=coords.device
                )
            if is_fixed_position is None:
                is_fixed_position = torch.zeros(
                    B, N, dtype=torch.long, device=coords.device
                )
            invs = invs + self.is_fixed_node_embed(is_fixed_node) * atom_mask.unsqueeze(
                -1
            )
            invs = invs + self.is_fixed_position_embed(
                is_fixed_position
...
class LigandGenerator(torch.nn.Module):
    ...
    def __init__(
        ...
        ligand_dec = LigandDecoder(
            ...
            use_is_fixed_embed=use_is_fixed_embed,
            use_mean_flow_delta_t=use_mean_flow_delta_t,
            eps=eps,
        )
    ...
    def forward(
        ...
        decoder_out = self.decode(
            ...
            mean_flow_delta_t=mean_flow_delta_t,
            cond_charges=cond_charges,
            cond_dist_edges=cond_dist_edges,
            is_fixed_node=is_fixed_node,
            is_fixed_position=is_fixed_position,
        )
