...
def main(args):
    ...
    vocab = util._build_vocab(virtual_nodes=getattr(args, "virtual_atom_p", 0.0) > 0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="semla",
        choices=["semla", "e3nn"],
        help="Backbone architecture: 'semla' (default Flowr) or 'e3nn' (cuEquivariance)",
    )
    parser.add_argument(
        "--e3nn_lmax", type=int, default=2, help="Max angular momentum for E3NN irreps"
    )
    parser.add_argument(
        "--e3nn_cutoff",
        type=float,
        default=5.0,
        help="Distance cutoff for E3NN edges (Angstrom)",
    )
    parser.add_argument(
        "--e3nn_cross_cutoff",
        type=float,
        default=10.0,
        help="Cross-attention distance cutoff (Angstrom)",
    )
    parser.add_argument(
        "--e3nn_hidden_channels",
        type=int,
        default=128,
        help="Hidden channels in E3NN MLPs",
    )
    parser.add_argument(
        "--e3nn_msg_channels",
        type=str,
        default=None,
        help="Comma-separated message channels per L for E3NN, e.g. '128,32,8'",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="stacking",
        choices=["stacking", "residual"],
    )
    parser.add_argument("--self_condition_prob", type=float, default=0.5)
    parser.add_argument("--sc_charges", action="store_true", default=False)
    parser.add_argument("--sc_distance_edges", action="store_true", default=False)
    parser.add_argument(
        "--sc_schedule",
        type=str,
        default="constant",
        choices=["constant", "linear_warmup", "cosine_warmup"],
    )
    parser.add_argument("--sc_warmup_steps", type=int, default=10000)
    parser.add_argument("--sc_recycle_steps", type=int, default=1)
    ...
    parser.add_argument(
        "--graph_inpainting_prob",
        type=float,
        default=0.15,
        help="Probability of graph inpainting mode during training (only when --graph_inpainting is set).",
    )
    parser.add_argument(
        "--skip_ot_for_graph_inpainting",
        action="store_true",
        default=True,
        help="Skip OT permutation matching for graph inpainting mode.",
    )
    parser.add_argument(
        "--harmonic_prior_sigma",
        type=float,
        default=1.0,
        help="Scale factor for harmonic prior coordinate generation.",
    )
    parser.add_argument(
        "--use_is_fixed_embed",
        action="store_true",
        help="Add per-atom is_fixed_node / is_fixed_position embeddings to the model.",
    )
    parser.add_argument(
        "--mask_fixed_discrete_loss",
        action="store_true",
        help="Exclude fixed atoms from discrete feature losses during training.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Proportion of virtual (NOATOM) atoms to add during training for flexible atom count. 0.0 disables.",
    )
    parser.add_argument(
        "--virtual_atom_noise_std",
        type=float,
        default=0.5,
        help="Std of Gaussian noise for virtual atom coordinates around center of mass.",
    )
