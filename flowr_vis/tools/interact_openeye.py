...
def main():
    ...
    if not oedepict.OEIsRegisteredImageFile(ext):
        logger.error("Unknown image type '%s'!", ext)
        return 1
    ...
    if not get_protein_and_ligand(protein, ligand, args):
        logger.error("Cannot initialize protein and/or ligand!")
        return 1
    ...
    logger.info("Successfully wrote interaction diagram to: %s", args.output)
    return 0
...
def get_protein_and_ligand(protein, ligand, args):  # NOSONAR
    """
    ...
    if args.complex:
        ...
        if not ifs.open(args.complex):
            logger.error("Cannot open complex file: %s", args.complex)
            return False

        complexmol = oechem.OEGraphMol()
        if not oechem.OEReadMolecule(ifs, complexmol):
            logger.error("Unable to read complex from %s", args.complex)
            return False
        ...
        if not split_complex(protein, ligand, complexmol, args.ligand_name):
            logger.error("Cannot separate complex into protein and ligand!")
            return False
    else:
        # Load protein from file
        ifs = oechem.oemolistream()
        if not ifs.open(args.protein):
            logger.error("Cannot open protein file: %s", args.protein)
            return False

        if not oechem.OEReadMolecule(ifs, protein):
            logger.error("Unable to read protein from %s", args.protein)
            return False

        # Load ligand from file, selecting by index
        ifs = oechem.oemolistream()
        if not ifs.open(args.ligand):
            logger.error("Cannot open ligand file: %s", args.ligand)
            return False
        ...
        if ligand.NumAtoms() == 0:
            logger.error(
                "Could not find ligand at index %d in %s. File contains %d molecule(s).",
                ligand_idx,
                args.ligand,
                current_idx,
            )
...
def write_complex_pdb(
    ...
    if method == "obabel":
        ...
        ret1 = subprocess.run(["obabel", sdf_file, "-O", lig_pdb], check=False)
        ret2 = subprocess.run(
            ["obabel", pdb_file, lig_pdb, "-O", out_path], check=False
        )
        ...
        try:
            ...
            logger.error("PyMOL not available. Install pymol or use --method obabel")
            return False
    else:
        logger.error("Unknown method '%s'. Use 'pymol' or 'obabel'.", method)
        return False
