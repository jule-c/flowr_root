...
# SD tag constants (shared across functions)
_SD_FINAL_ENERGY = "Final Energy"
_SD_ZAP_PBSA_BINDING = "Zap PBSA Binding Energy"
_SD_POSE_IDX = "Pose Idx"
...
def setup_openeye_license(license_filename: str = LICENSE_FILENAME) -> None:
    ...
    if os.path.isfile(license_filename):
        with open(license_filename, "r", encoding="utf-8") as license_file:
...
try:
    ...
    logger.warning("OpenEye license setup failed: %s", e)
...
def prep_structure(  # NOSONAR
    ...
    if handle_alt_locations:
        ...
        if alf.GetGroupCount() > 0:
            if verbose:
                logger.info(
                    "  Found %d alternate location groups, keeping highest occupancy",
                    alf.GetGroupCount(),
                )
    ...
    if build_sidechains:
        ...
        if oespruce.OEBuildSidechains(mol_copy, sidechain_opts):
            if verbose:
                logger.info("  Built missing sidechains")
        elif verbose:
            logger.info("  No sidechains needed building or build failed")
    ...
    if cap_termini:
        ...
        if verbose and (capped_n or capped_c):
            logger.info("  Capped termini (N: %s, C: %s)", capped_n, capped_c)
    ...
    for design_unit in design_units:
        ...
        if protonate:
            ...
            if verbose:
                logger.info("  Protonated design unit with flip optimization")
    ...
    if verbose:
        logger.info("  Created %d design unit(s)", len(dus))
...
def extract_best_receptor(dus: List) -> Optional[oechem.OEDesignUnit]:
    ...
    for du in dus:
        ...
        iridium_category = iridium_data.GetCategory()
        iridium_class = oechem.OEGetIridiumCategoryName(iridium_category)
        logger.info("Iridium class: %s", iridium_class)
        if iridium_class == "HT":
            return du
...
def gen_confs(mol: oechem.OEMol) -> Optional[oechem.OEMol]:
    ...
    for isomer in OEFlipper(mol, flipper_opts):
        ...
        if ret_code == OEOmegaReturnCode_Success:
            ...
            logger.error("Omega Failed because %s", OEGetOmegaError(ret_code))
            return None
    return None
...
def dock_ligand_hybrid(
    ...
    if ret_code == OEDockingReturnCode_Success:
        ...
        logger.error(
            "Docking Failed because %s",
            oedocking.OEDockingReturnCodeGetName(ret_code),
        )
        return None
...
def minimize_poses(  # NOSONAR
    ...
    for i, pose in enumerate(poses.GetConfs()):
        ...
        if ret_code == OESzybkiReturnCode_Success:
            ...
            OESetSDData(pose, _SD_FINAL_ENERGY, str(final_energy))
            OESetSDData(pose, "RMSD", str(ligand_rmsd))
            OESetSDData(pose, "Minimization", "Success")

            if verbose:
                logger.info(
                    "Minimization successful for pose %d: Initial Energy=%.2f, "
                    "Final Energy=%.2f, RMSD=%.3f",
                    i,
                    initial_energy,
                    final_energy,
                    ligand_rmsd,
            ...
            if filter_positive_energy and final_energy >= 0:
                ...
                if verbose:
                    logger.info(
                        "  -> Filtering pose %d (positive energy: %.2f)",
                        i,
                        final_energy,
                    )
            else:
            ...
            if verbose:
                logger.warning("Minimization failed for pose %d: %s", i, error_name)

    if failed_count > 0:
        logger.warning(
            "Minimization: %d conformers failed (last error: %s)",
            failed_count,
            last_error,
        )
    ...
    if filter_positive_energy and positive_energy_count > 0:
        logger.info(
            "Minimization: Filtered %d conformers with positive final energy",
    ...
    if not confs_to_keep:
        if filter_positive_energy:
            logger.warning(
                "No conformers with negative energy remain after minimization! Keeping lowest five energy conformers."
            )
            # Keep the five conformers with lowest final energy
            energy_conf_pairs = []
            for pose in poses.GetConfs():
                final_energy_str = OEGetSDData(pose, _SD_FINAL_ENERGY)
                try:
            ...
            logger.warning(
                "Minimization failed for ALL conformers! Returning unminimized poses."
            )
            return poses
...
def minimize_poses_flex(  # NOSONAR
    du: oechem.OEDesignUnit,
    poses: oechem.OEMol,
    ...
    if failed_count > 0:
        logger.warning(
            "Flex minimization: %d conformers failed (last error: %s)",
            failed_count,
            last_error,
        )
    ...
    if filter_positive_energy and positive_energy_count > 0:
        logger.info(
            "Flex minimization: Filtered %d conformers with positive final energy",
            positive_energy_count,
        )
    ...
    if not confs_to_keep:
        if filter_positive_energy:
            logger.warning(
                "No conformers with negative energy remain after flex minimization!"
            )
            return None, None
        else:
            logger.warning(
...
def calc_rmsd_mcs(
    ...
    # Find best MCS match
    best_rmsd = float("inf")
    for match in mcss.Match(fit, True):  # param: uniquify=True
...
def calc_rmsd(
    ...
    if ref.NumAtoms() == fit.NumAtoms():
        ...
        rmsd = oechem.OERMSD(ref, fit, True)  # param: automorph=True
        if rmsd >= 0:
            return rmsd
        ...
        rmsd = oechem.OERMSD(
            ref, fit, True, True
        )  # param: automorph=True, overlay=True
        if rmsd >= 0:
            return rmsd
...
def select_top_poses_by_rmsd(
    ...
    for mol_idx, mol in enumerate(poses_list):
        ...
        for conf_idx, conf in enumerate(mol.GetConfs()):
            ...
            if rmsd == float("inf"):
                logger.warning(
                    "RMSD calculation failed for mol %d, conf %d",
                    mol_idx,
                    conf_idx,
                )
    ...
    logger.info(
        "RMSD calculation summary: %d/%d poses have valid RMSD",
        valid_count,
        len(all_poses_with_rmsd),
    )
...
def cluster_poses_by_rmsd(  # NOSONAR
    poses: oechem.OEMol,
    eps: float = 1.5,
    min_samples: int = 2,
    ...
    for label in unique_labels:
        if label == -1:
            ...
            if include_noise:
                noise_indices = npnonzero(labels == label)[0]
            ...
            indices = npnonzero(labels == label)[0]
...
def select_diverse_poses_by_rmsd(  # NOSONAR
    poses_list: List[oechem.OEMol],
    top_n: int,
...
def calc_zap_pbsa_binding_single_pose(
    ...
    OESetSDData(pose_mol, _SD_ZAP_PBSA_BINDING, str(binding_energy))
    OESetSDData(pose_mol, _SD_POSE_IDX, str(pose_idx))
...
def calc_zap_pbsa_binding_parallel(
    ...
    logger.info("  Calculating binding energies for %d poses...", len(all_poses))
    ...
    logger.info("  Processed %d molecules with binding energies", len(output_list))
    return output_list
...
def calc_zap_pbsa_binding(  # NOSONAR
    du: oechem.OEDesignUnit,
    ligand: oechem.OEMol,
    multiple_poses: bool,
    charge_type: str = CHARGE_TYPE_MMFF94,
    ...
    if multiple_poses:
        for pose in ligand.GetConfs():
            ...
            binding_energy = results.GetBindingEnergy()
            OESetSDData(pose, _SD_ZAP_PBSA_BINDING, str(binding_energy))
            OESetSDData(pose, _SD_POSE_IDX, str(pose_idx))
    else:
        ...
        binding_energy = results.GetBindingEnergy()
        OESetSDData(ligand, _SD_ZAP_PBSA_BINDING, str(binding_energy))
...
def select_top_poses_mm_pbsa(poses: oechem.OEMol, top_n: int = 5) -> List[oechem.OEMol]:
    ...
    energies, idx = [], []
    for pose in poses.GetConfs():
        energy = float(oechem.OEGetSDData(pose, _SD_ZAP_PBSA_BINDING))
        pose_idx = int(oechem.OEGetSDData(pose, _SD_POSE_IDX))
        energies.append(energy)
        idx.append(pose_idx)

    df = pd.DataFrame({_SD_POSE_IDX: idx, _SD_ZAP_PBSA_BINDING: energies})
    sorted_df = df.sort_values(by=_SD_ZAP_PBSA_BINDING, ascending=True)
    selected_indices = sorted_df.iloc[:top_n]

    top_poses = []
    for selected_idx in selected_indices[_SD_POSE_IDX]:
...
def merge_ligand_with_pdb(  # NOSONAR
    protein_file: str,
    ligand_file: str,
    output_file: str,
    ...
    logger.info(
        "Merged %d ligand atoms (%s) with protein (%d atoms) -> %s",
        num_atoms,
        ligand_res_name,
        len(protein_array),
        output_file,
...
def split_ligand_from_pdb(  # NOSONAR
    ...
    if pdb_id and not in_file:
        logger.info("Downloading PDB structure for ID: %s", pdb_id)
    ...
    logger.info("Non-standard residues found in the protein: %s", nonstd_res)
    ...
    if exclude_res is None:
        ...
        for res in nonstd_res:
            if res not in keep_residues:
                exclude_res.add(res)
        logger.info("Auto-detected ligand residues to exclude: %s", exclude_res)
    ...
    if not exclude_res:
        logger.warning(
            "No ligand residues identified to exclude. Output will be identical to input."
        )
    ...
    logger.info("Kept %d atoms after excluding ligand residues", len(prot_atom_array))

    # Check water molecules specifically
    water_mask = np.isin(prot_atom_array.res_name, ["HOH", "WAT", "H2O"])
    if water_mask.sum() > 0:
        logger.info("Found %d water molecules in protein structure.", water_mask.sum())
        logger.info(
            "Water chain IDs: %s", np.unique(prot_atom_array.chain_id[water_mask])
        )
        logger.info("Water res IDs: %s", np.unique(prot_atom_array.res_id[water_mask]))
    ...
    if save_ligand:
        ...
        logger.info(
            "Saved ligand-only structure with %d atoms to %s",
            len(ligand_atom_array),
            ligand_out_file,
        )
...
def split_other_from_pdb(
    ...
    nonstd_res = [x for x in res_names if x not in aa_names]
    logger.info("Non-standard residues found: %s", nonstd_res)
    ...
    logger.info("Identified ligand residues: %s", ligand_residues)
    logger.info("'Other' residues to remove: %s", residues_to_remove)

    # If not keeping ligand, add ligand residues to removal set
    if not keep_ligand:
        residues_to_remove.update(ligand_residues)
        logger.info("Also removing ligand residues (keep_ligand=False)")
    ...
    logger.info("Removed %d atoms from 'other' components", removed_count)
    logger.info("Kept %d atoms (protein + water + ligand)", len(clean_atom_array))
    ...
    logger.info("Cleaned structure saved to %s", out_file)
