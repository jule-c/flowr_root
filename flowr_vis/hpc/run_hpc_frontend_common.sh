#!/bin/bash
# ══════════════════════════════════════════════════════════════════════
#  FLOWR Frontend Server — Common Logic
# ══════════════════════════════════════════════════════════════════════
# This file is NOT meant to be run directly. It is sourced by the
# cluster-specific wrapper scripts (run_hpc_frontend.sh, etc.) after
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            echo "Usage: $0 [--config <path-to-hpc.env>]"
            echo ""
...
export FLOWR_SLURM_WORKER_SCRIPT="${HPC_DIR}/${WORKER_SCRIPT_NAME}"
...
export FLOWR_SLURM_PARTITION="${FLOWR_SLURM_PARTITION:-${DEFAULT_PARTITION}}"
...
# Pass config file path so the worker script can source the same settings
