...
def mol_to_smiles(mol) -> str:
    ...
    try:
        ...
        logger.debug("Failed to convert mol to SMILES", exc_info=True)
        return ""
...
def validate_molecule(mol) -> tuple:
    """Validate that an RDKit molecule is chemically sane.

    Checks sanitization (kekulization, valence, etc.) and basic structure.
    ...
    if mol is None:
        return False, "Molecule could not be parsed"

    # Work on a copy so the original mol isn't modified
    ...
    try:
        ...
        if "kekulize" in msg.lower() or "Can't kekulize" in msg:
            return False, (
                "Molecule has invalid aromatic system (kekulization failed). "
                "Please check the bond orders and aromatic assignments."
            )
        if "valence" in msg.lower():
            return False, (
                "Molecule has atoms with invalid valence. "
                "Please check the atom types and bond orders."
            )
        return False, f"Molecule failed chemical validation: {msg}"

    # Check for atoms with no coordinates (all zeros is suspicious but allowed)
...
def compute_all_properties(mol) -> Dict[str, Any]:
    ...
    for name, func in CONT_PROPERTIES_RDKIT.items():
        try:
            ...
            logger.debug("Failed to compute property %s", name, exc_info=True)
            props[name] = None
    ...
    try:
        ...
        logger.debug("Failed to compute NumChiralCenters", exc_info=True)
...
def mol_to_morgan_fp(mol, radius: int = 2, n_bits: int = 2048) -> Optional[np.ndarray]:
    ...
    try:
        ...
        try:
            ...
            logger.debug("Fallback Morgan FP computation failed", exc_info=True)
            return None
    except Exception:
        logger.debug("Morgan FP computation failed", exc_info=True)
        return None
...
def crawl_sdf_affinity(filepath: str) -> Optional[Dict[str, Any]]:  # NOSONAR
    ...
    try:
        ...
        logger.debug("Failed to read SDF file %s", filepath, exc_info=True)
        return None
...
def compute_pocket_com(  # NOSONAR
    protein_path: str,
    ligand_path: str,
    ...
    try:
        ...
        logger.debug(
            "Failed to extract ligand coordinates from %s", ligand_path, exc_info=True
        )
        return None
    ...
    try:
        ...
        with open(protein_path, "r", encoding="utf-8") as fh:
            for line in fh:
        ...
        logger.debug("Failed to parse protein PDB %s", protein_path, exc_info=True)
        return None
